...
#define ENDMsg(t)                                 \
	gtest_lite::test.end(true) << t << std::endl; \
	}
...
#define CREATE_Has_(X)                                         \
	...
	struct _Has_##X {                                          \
		...
		struct ChT;                                            \
		template <typename D>                                  \
		static char (&f(ChT<int Fallback::*, &D::X>*))[1];     \
		template <typename D>                                  \
		static char (&f(...))[2];                              \
...
private:  /// singleton minta miatt
	Test()
	    : sum(0), failed(0), status(false), null("/dev/null") {}
	Test(const Test&);
...
std::ostream& EXPECT_(T1* exp, std::nullptr_t act, bool (*pred)(T1*, std::nullptr_t), const char* file, int line, const char* expr, const char* lhs = "elvart", const char* rhs = "aktual") {
	return test.expect(pred(exp, act), file, line, expr)
	       << "** " << lhs << ": " << (void*)exp
	       << "\n** " << rhs << ": " << (void*)act << std::endl;
}
...
inline std::ostream& EXPECTSTR(const char* exp, const char* act, bool (*pred)(const char*, const char*), const char* file, int line, const char* expr, const char* lhs = "elvart", const char* rhs = "aktual") {
	return test.expect(pred(exp, act), file, line, expr)
...
inline bool nestr(const char* a, const char* b) {
	if (a != NULL && b != NULL)
		return strcmp(a, b) != 0;
	return false;
}
